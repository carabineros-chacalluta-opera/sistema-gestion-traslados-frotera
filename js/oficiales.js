// ═══════════════════════════════════════════════════════════════
// SIGDOF · Módulo Oficiales (CRUD + Historial) v2.0
// ═══════════════════════════════════════════════════════════════

const Oficiales = {
  data: [], cuarteles: [], filtro: '', editandoId: null,
  formacionItems: [],
  ...
  renderBolsa() {
    ...
    if (!cont) return;
    // CORREGIDO: usa Utils.esEnBolsa centralizado
    const bolsa = this.data.filter(o => Utils.esEnBolsa(o));
    if (!bolsa.length) {
  ...
  async verFicha(id) {
    ...
    // Cargar historial de movimientos del oficial
    const { data: historial } = await sb.from('movimientos')
      .select('*, cuarteles_destino:cuartel_destino_id(nombre), cuarteles_origen:cuartel_origen_id(nombre), planes_traslado:plan_id(anio)')
      .eq('oficial_id', id)
    ...
    document.getElementById('ficha-content').innerHTML = `
      ...
      <div style="margin-top:20px;">
        <div style="font-family:'Oswald',sans-serif;font-size:0.9rem;font-weight:600;color:#263238;text-transform:uppercase;margin-bottom:12px;padding-bottom:8px;border-bottom:2px solid #E8F5E9;">
          📋 Historial de Movimientos
        </div>
        ${historial && historial.length ? `
        <div style="position:relative;padding-left:24px;">
          <div style="position:absolute;left:8px;top:0;bottom:0;width:2px;background:#E8F5E9;"></div>
          ${historial.map(m => `
          <div style="position:relative;margin-bottom:14px;">
            <div style="position:absolute;left:-20px;top:4px;width:12px;height:12px;background:${m.tipo==='SE_MUEVE'?'#1565C0':'#2E7D32'};border-radius:50%;border:2px solid white;box-shadow:0 0 0 2px ${m.tipo==='SE_MUEVE'?'#1565C0':'#2E7D32'};"></div>
            <div style="background:#f9f9f9;border:1px solid #e0e0e0;border-radius:8px;padding:10px 12px;">
              <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;">
                <span class="badge ${m.tipo==='SE_MUEVE'?'badge-azul':'badge-verde'}" style="font-size:0.68rem;">${m.tipo.replace('_',' ')}</span>
                <span style="font-size:0.78rem;color:#546E7A;">Plan ${m.planes_traslado?.anio||'—'}</span>
                ${m.es_excepcion?'<span class="badge badge-amarillo" style="font-size:0.68rem;">⚠️ Excepción</span>':''}
              </div>
              <div style="font-size:0.82rem;">
                ${m.cuarteles_origen?.nombre?`<span style="color:#546E7A;">Desde:</span> <b>${m.cuarteles_origen.nombre}</b> → `:''}
                <span style="color:#546E7A;">Hacia:</span> <b>${m.cuarteles_destino?.nombre||'—'}</b>
                ${m.cargo_destino?` · <span style="color:#546E7A;">${CARGOS_LABEL[m.cargo_destino]||m.cargo_destino}</span>`:''}
              </div>
              ${m.permanencia_proyectada?`<div style="font-size:0.75rem;color:#546E7A;margin-top:2px;">Permanencia proyectada: ${m.permanencia_proyectada.toFixed(1)} años</div>`:''}
            </div>
          </div>`).join('')}
        </div>` : '<div style="font-size:0.85rem;color:#999;padding:8px 0;">Sin movimientos registrados en el sistema.</div>'}
      </div>
  ...
  async darDeBaja(id) {
    ...
    const { error } = await sb.from('oficiales').update({
      estado:'BAJA', cuartel_id:null, cargo_actual:null, updated_at:new Date().toISOString()
    }).eq('id', id);
