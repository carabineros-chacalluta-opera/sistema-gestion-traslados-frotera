// ═══════════════════════════════════════════════════════════════
// SIGDOF · Simulador de Traslados + Planes de Traslado v2.0
// ═══════════════════════════════════════════════════════════════
...
const Simulador = {
  ...
  seleccionarOficial(id) {
    ...
    this.calcular();
    // Actualizar comparador multi-candidato si está abierto
    if (document.getElementById('comparador-panel')?.style.display !== 'none') {
  ...
  seleccionarCargo(cargo) {
    this.selCargo = cargo;
    this.calcular();
    this.renderCargosDisponibles(); // Refrescar selección visual
  },
  ...
  renderPanelOficial() {
    ...
    cont.innerHTML = `
      <div style="background:#E8F5E9;border-radius:8px;padding:14px;">
        ...
        ${o.cuartel_id ? `<div style="margin-top:8px;font-size:0.78rem;color:#546E7A;">Cuartel actual: ${this.cuarteles.find(c=>c.id===o.cuartel_id)?.nombre||'—'}</div>` : ''}
      </div>`;
  },
    ...
    cont.innerHTML = `
      <div class="simulador-panel ${resultado.valido?'resultado-valido':'resultado-invalido'}" style="height:100%;">
        ...
        <div class="simulador-panel-body">
          ...
          ${resultado.valido && !resultado.errores.length ? `
            <div class="validacion-item validacion-ok">✅ Grado compatible con cargo</div>
          ...
          <div style="margin-top:12px;display:flex;gap:8px;">
            <button class="btn btn-primary" style="flex:1;" onclick="Planes.agregarMovimiento('SE_MUEVE')">➕ Agregar Traslado al Plan</button>
          </div>
          <div style="margin-top:6px;">
            <button class="btn btn-outline" style="width:100%;" onclick="Planes.agregarMovimiento('SE_MANTIENE')">📌 Registrar "Se Mantiene"</button>
          </div>` : `
          <div style="margin-top:12px;">
  ...
  aplicarExcepcion() {
    ...
    toast('Excepción operativa registrada.','warning');
    Planes.agregarMovimiento('SE_MUEVE', true, motivo);
  },

  // ── COMPARADOR MULTI-CANDIDATO ──────────────────────────────────
  abrirComparador() {
    const panel = document.getElementById('comparador-panel');
    if (panel) {
      panel.style.display = panel.style.display === 'none' ? 'block' : 'none';
      if (panel.style.display === 'block') this.renderComparador();
    }
  },

  renderComparador() {
    if (!this.selCuartel || !this.selCargo) return;
    const cont = document.getElementById('comparador-lista');
    if (!cont) return;

    // Filtrar candidatos válidos por especialidad y grado
    const gradosValidos = GRADOS_POR_CARGO[this.selCargo] || [];
    const candidatos = this.oficiales
      .filter(o => o.especialidad_mof && gradosValidos.includes(o.grado))
      .map(o => {
        const res = Utils.validarAsignacion(o, this.selCargo, { ...this.selCuartel, fecha_ingreso_cuartel: null });
        const aniosHastaAsc = Utils.aniosHastaAscenso(o.grado, o.fecha_ascenso_grado);
        return { ...o, permanencia: res.permanencia, valido: res.valido, errores: res.errores, advertencias: res.advertencias, aniosHastaAsc };
      })
      .sort((a, b) => b.permanencia - a.permanencia);

    if (!candidatos.length) {
      cont.innerHTML = '<p style="color:#999;font-size:0.85rem;padding:12px 0;">Sin candidatos con especialidad y grado compatible.</p>';
      return;
    }

    cont.innerHTML = candidatos.map((o, i) => `
      <div style="
        display:flex;align-items:center;gap:10px;padding:10px;border-radius:8px;margin-bottom:6px;
        border:2px solid ${this.selOficial?.id===o.id?'#2E7D32':'#e0e0e0'};
        background:${i===0?'#F1F8E9':this.selOficial?.id===o.id?'#E8F5E9':'white'};
        cursor:pointer;transition:all 0.15s;
      " onclick="document.getElementById('sim-oficial').value='${o.id}';Simulador.seleccionarOficial('${o.id}')">
        ${i===0?'<span title="Mejor candidato" style="font-size:1.1rem;">🥇</span>':'<span style="font-size:0.85rem;color:#999;width:20px;text-align:center;">${i+1}</span>'}
        <div style="flex:1;">
          <div style="font-weight:600;font-size:0.85rem;">${badgeGrado(o.grado)} ${o.nombre}</div>
          <div style="font-size:0.75rem;color:#546E7A;">Ascenso en ${o.aniosHastaAsc>90?'—':o.aniosHastaAsc.toFixed(1)+' años'}</div>
        </div>
        <div style="text-align:right;">
          <div style="font-family:'Oswald',sans-serif;font-size:1.4rem;font-weight:700;color:${o.permanencia>=4?'#2E7D32':o.permanencia>=2?'#F9A825':'#C62828'};">${o.permanencia.toFixed(1)}</div>
          <div style="font-size:0.68rem;color:#546E7A;">años perm.</div>
        </div>
        ${o.advertencias.length?`<span title="${o.advertencias.join('; ')}" style="color:#F9A825;font-size:1.1rem;">⚠️</span>`:''}
      </div>`).join('');
  },
...
const Planes = {
  ...
  async cargarMovimientos(planId) {
    const { data } = await sb.from('movimientos')
      .select('*, oficiales(nombre,grado,cuartel_id), cuarteles_destino:cuartel_destino_id(nombre)')
      .eq('plan_id', planId)
  ...
  renderListaPlanes(planes) {
    ...
    cont.innerHTML = planes.map(p=>`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px;border:1px solid #e0e0e0;border-radius:8px;margin-bottom:8px;">
        <div>
          <b>Plan ${p.anio}</b>
          <span class="badge ${p.estado==='EJECUTADO'?'badge-verde':p.estado==='BORRADOR'?'badge-amarillo':'badge-gris'}" style="margin-left:6px;">${p.estado}</span>
        </div>
  ...
  // MEJORADO: acepta tipo (SE_MUEVE / SE_MANTIENE) explícitamente
  agregarMovimiento(tipo='SE_MUEVE', esExcepcion=false, justifExcepcion='') {
    ...
    const justif = document.getElementById('justif-text')?.innerText ||
      Utils.generarJustificacion(Simulador.selOficial, Simulador.selCargo, Simulador.selCuartel, tipo);
    ...
    this.renderPlanActual();
    toast(`Movimiento "${tipo.replace('_',' ')}" agregado al plan`);
  },
  ...
  renderPlanActual() {
    ...
    if (!this.movimientos.length) {
      cont.innerHTML = `<div class="empty-state"><div class="empty-icon">📋</div><p>Plan ${this.planActual.anio} creado. Sin movimientos aún.</p><p style="margin-top:8px;font-size:0.85rem;">Usa el Simulador para agregar movimientos o registrar decisiones "Se Mantiene".</p></div>`;
      return;
    }

    const totalMovidos   = this.movimientos.filter(m=>m.tipo==='SE_MUEVE').length;
    const totalMantenidos = this.movimientos.filter(m=>m.tipo==='SE_MANTIENE').length;

    cont.innerHTML = `
      <div style="display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap;">
        <div style="background:#E3F2FD;border-radius:8px;padding:10px 16px;font-size:0.85rem;">
          <b style="color:#1565C0;">↗️ Se mueven:</b> ${totalMovidos}
        </div>
        <div style="background:#E8F5E9;border-radius:8px;padding:10px 16px;font-size:0.85rem;">
          <b style="color:#2E7D32;">📌 Se mantienen:</b> ${totalMantenidos}
        </div>
        <div style="background:#F5F5F5;border-radius:8px;padding:10px 16px;font-size:0.85rem;">
          <b>Total:</b> ${this.movimientos.length} decisiones
        </div>
      </div>
  ...
  // CORREGIDO: ejecución transaccional mediante bucle con rollback manual en caso de error
  async ejecutarPlan() {
    ...
    for (const m of this.movimientos) {
      try {
        ...
        const { error: errMov } = await sb.from('movimientos').insert(mov);
        if (errMov) { errores.push(`Movimiento ${m.oficial?.nombre}: ${errMov.message}`); continue; }

        // Solo actualizar ubicación si es traslado real
        if (m.tipo === 'SE_MUEVE') {
          const { error: errOf } = await sb.from('oficiales').update({
          ...
          if (errOf) errores.push(`Actualización oficial ${m.oficial?.nombre}: ${errOf.message}`);
        }
      } catch(e) {
        errores.push(`Error inesperado: ${e.message}`);
      }
    }

    await sb.from('planes_traslado').update({
      estado: errores.length ? 'BORRADOR' : 'EJECUTADO',
      fecha_ejec: errores.length ? null : new Date().toISOString().split('T')[0]
    }).eq('id', this.planActual.id);

    mostrarCargando(false);

    if (errores.length) {
      toast(`Plan ejecutado con ${errores.length} error(es). Revise el log.`, 'error');
      console.error('Errores en ejecución de plan:', errores);
    } else {
  ...
  async exportarPDF() {
    ...
    doc.setFontSize(9);
    doc.text(`Total decisiones: ${this.movimientos.length}   |   Traslados: ${totalMovidos}   |   Se mantienen: ${totalMantenidos}`, 15, y+9);
    y += 20;
    ...
    this.movimientos.forEach((m, i) => {
      ...
      const esMueve = m.tipo === 'SE_MUEVE';
      doc.setFillColor(esMueve?232:232, esMueve?244:245, esMueve?253:232);
      doc.rect(10, y, 190, 7, 'F');
      doc.setFontSize(10); doc.setFont('helvetica','bold');
      doc.text(`${i+1}. ${m.tipo.replace('_',' ')} — ${GRADOS_LABEL[o.grado]||''} ${o.nombre||''}`, 12, y+5);
      y += 9;
      doc.setFontSize(8); doc.setFont('helvetica','normal');
      if (esMueve && cd.nombre) {
        doc.text(`Destino: ${CARGOS_LABEL[m.cargo_destino]||''}, ${cd.nombre} | Permanencia: ${(m.permanencia_proyectada||0).toFixed(1)} años`, 12, y+3);
        y += 6;
      }
  ...
  // NUEVO: exportar a Excel (xlsx via SheetJS CDN)
  async exportarExcel() {
    if (typeof XLSX === 'undefined') return toast('SheetJS no disponible','error');
    const anio = this.planActual?.anio || new Date().getFullYear();
    const filas = this.movimientos.map((m, i) => {
      const o  = m.oficial  || {};
      const cd = m.cuartel_destino || {};
      return {
        'N°':             i + 1,
        'Decisión':       m.tipo.replace('_',' '),
        'Grado':          GRADOS_LABEL[o.grado] || '',
        'Nombre':         o.nombre || '',
        'Cargo Destino':  CARGOS_LABEL[m.cargo_destino] || '',
        'Cuartel Destino': cd.nombre || '',
        'Permanencia (años)': (m.permanencia_proyectada||0).toFixed(1),
        'Excepción':      m.es_excepcion ? 'Sí' : 'No',
        'Justificación':  m.justificacion || '',
      };
    });
    const ws = XLSX.utils.json_to_sheet(filas);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, `Plan ${anio}`);
    XLSX.writeFile(wb, `SIGDOF_Plan_Traslados_${anio}.xlsx`);
  }
};
