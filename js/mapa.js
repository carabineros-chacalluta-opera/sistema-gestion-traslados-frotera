...
const Mapa = {
  // CORRECCIÓN #3: dos instancias independientes para evitar conflicto
  mapDashboard: null,
  markers: [],
  markersFull: [],
  filtroActual: 'todos',

  // Inicializa el mapa del dashboard sobre su propio div
  initDashboard(divId) {
    if (this.mapDashboard) return;
    this.mapDashboard = L.map(divId, { center: [-35.5, -71.0], zoom: 5, zoomControl: true });
  ...
  // Renderiza en el mapa del dashboard (llamado desde Dashboard.cargar)
  renderizar(cuarteles, oficiales) {
    if (!this.mapDashboard) return;
    this.markers.forEach(m => m.remove());
    this.markers = [];
    this._poblarMapa(this.mapDashboard, this.markers, cuarteles, oficiales);
  },

  // Renderiza en el mapa full de la página Mapa Operativo
  renderizarEnMapa(mapaInstancia, cuarteles, oficiales) {
    this.markersFull.forEach(m => m.remove());
    this.markersFull = [];
    this._poblarMapa(mapaInstancia, this.markersFull, cuarteles, oficiales);
  },

  // Lógica compartida de renderizado de markers
  ...
  aplicarFiltro() {
    // Aplica el filtro a ambos conjuntos de markers
    [...this.markers, ...this.markersFull].forEach(m => {
