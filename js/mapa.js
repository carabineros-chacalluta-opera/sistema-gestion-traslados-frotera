// ═══════════════════════════════════════════════════════════════
// SIGDOF · Mapa Interactivo de Chile (Leaflet.js) v2.0
// ═══════════════════════════════════════════════════════════════
...
const Mapa = {
  ...
  // CORREGIDO: mapa full tiene su propia instancia y sus propios markers
  initFull(divId) {
    if (this.mapFull) return;
    this.mapFull = L.map(divId, { center: [-35.5, -71.0], zoom: 5, zoomControl: true });
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      attribution: '© OpenStreetMap', maxZoom: 18
    }).addTo(this.mapFull);
    this.initFiltros();
  ...
  // CORREGIDO: renderiza en el mapa full con sus propios markers
  renderizarFull(cuarteles, oficiales) {
    ...
    this.aplicarFiltro(); // Reaplicar filtro activo
  },
  ...
  // CORREGIDO: aplica filtro solo a los markers del mapa full (el dashboard no tiene filtros)
  aplicarFiltro() {
