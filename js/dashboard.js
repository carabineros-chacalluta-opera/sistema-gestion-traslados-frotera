...
const Dashboard = {
  ...
  renderStats() {
    ...
    const fueraDeFrontera = especialistas.filter(o => !o.cuartel_id || o.estado !== 'ACTIVO');
    ...
    stats.forEach(s => {
      ...
      if (el) { el.textContent = s.val; this.animarNumero(el, s.val); }
    });
  },
  ...
  generarAlertas() {
    ...
    this.cuarteles.forEach(c => {
      ...
      if (especialistasEnCuartel.length === 0 && asignados.length > 0) {
        this.alertas.push({ nivel: c.criticidad==='ALTA'?'critica':'preventiva', cuartel: c.nombre, msg:`Sin ningún especialista "Montaña o Frontera" en dotación activa.`, icon: c.criticidad==='ALTA'?'🔴':'🟡' });
      }
    });
    ...
    this.oficiales.forEach(o => {
      ...
      if (restante < -0.25) {
        this.alertas.push({ nivel:'critica', oficial: o.nombre, msg:`${GRADOS_LABEL[o.grado]} ${o.nombre}: rotación VENCIDA en ${c.nombre} (lleva ${acumulado.toFixed(1)} años, límite ${limite} años).`, icon:'🔴' });
      } else if (restante <= 0.5 && restante >= 0) {
        this.alertas.push({ nivel:'preventiva', oficial: o.nombre, msg:`${GRADOS_LABEL[o.grado]} ${o.nombre}: rotación próxima en ${Math.round(restante*12)} meses en ${c.nombre}.`, icon:'🟡' });
      }
      ...
      if (gradoSig && o.cargo_actual) {
        ...
        if (compActual && !compSig && aniosHastaAsc <= 1) {
          this.alertas.push({ nivel:'critica', oficial: o.nombre, msg:`${GRADOS_LABEL[o.grado]} ${o.nombre}: ascenderá a ${GRADOS_LABEL[gradoSig]} en ~${Math.round(aniosHastaAsc*12)} meses y quedará incompatible con cargo actual (${CARGOS_LABEL[o.cargo_actual]}).`, icon:'🔴' });
        } else if (compActual && !compSig && aniosHastaAsc <= 2) {
          this.alertas.push({ nivel:'preventiva', oficial: o.nombre, msg:`${GRADOS_LABEL[o.grado]} ${o.nombre}: en ~${Math.round(aniosHastaAsc*12)} meses ascenderá a ${GRADOS_LABEL[gradoSig]}, incompatible con ${CARGOS_LABEL[o.cargo_actual]}.`, icon:'🟡' });
        }
      }
      ...
      if (o.estado === 'PERFECCIONAMIENTO' && o.acipol_retorno) {
        ...
        if (diasRetorno <= 90 && diasRetorno > 0) {
          this.alertas.push({ nivel:'preventiva', oficial: o.nombre, msg:`${GRADOS_LABEL[o.grado]} ${o.nombre}: retorna de perfeccionamiento en ${Math.round(diasRetorno)} días. Planificar destino.`, icon:'🟡' });
        }
      }
    });
    ...
    if (badge) badge.textContent = this.alertas.filter(a=>a.nivel==='critica').length || '';

    const navBadge = document.getElementById('nav-alertas-badge');
    if (navBadge) navBadge.textContent = this.alertas.length;
  },
...
const Charts = {
  ...
  renderDashboard(cuarteles, oficiales) {
    ...
    this.renderBarrasRegion(cuarteles, oficiales);
  ...
  renderDonaEspecialistas(oficiales) {
    ...
    const bolsa      = oficiales.filter(o => o.especialidad_mof && (!o.cuartel_id || o.estado!=='ACTIVO') && o.estado!=='BAJA' && o.estado!=='RETIRADO').length;
