// ═══════════════════════════════════════════════════════════════
// SIGDOF · Dashboard y Motor de Alertas v2.0
// ═══════════════════════════════════════════════════════════════
...
const Dashboard = {
  ...
  renderStats() {
    ...
    const fueraDeFrontera = this.oficiales.filter(o => Utils.esEnBolsa(o));
  ...
  calcularVacantes() {
    ...
    this.cuarteles.forEach(c => {
      ...
      total += Math.max(0, Utils.dotacionOptima(c) - asignados.length);
    });
    return total;
  },
  ...
  generarAlertas() {
    ...
    this.cuarteles.forEach(c => {
      ...
      if (deficit > 0 && c.criticidad === 'ALTA') {
        ...
        this.alertas.push({ nivel:'preventiva', cuartel: c.nombre, msg:`${deficit} cargo(s) vacante(s) en ${c.nombre}.`, icon:'🟡' });
      }
    ...
    this.oficiales.forEach(o => {
      ...
      if (restante < -0.25) {
        this.alertas.push({
          ...
          msg: `${GRADOS_LABEL[o.grado]} ${o.nombre}: rotación VENCIDA en ${c.nombre} (${exceso.toFixed(1)} año(s) de exceso sobre el límite de ${limite} años).`,
          icon: '🔴'
        });
      ...
      if (gradoSig && o.cargo_actual) {
        ...
        if (compActual && !compSig && aniosHastaAsc <= 1) {
          this.alertas.push({
            nivel: 'critica', oficial: o.nombre,
            msg: `${GRADOS_LABEL[o.grado]} ${o.nombre}: ascenderá a ${GRADOS_LABEL[gradoSig]} en ~${Math.round(aniosHastaAsc * 12)} meses e quedará incompatible con ${CARGOS_LABEL[o.cargo_actual]}.`,
            icon: '🔴'
          });
    ...
    if (mes === 4) {
      this.alertas.unshift({ nivel: 'info', msg: 'Abril: Es momento de iniciar la programación del Plan de Traslados de diciembre.', icon: '📅' });
    } else if (mes === 11) {
    ...
    if (navBadge) {
      navBadge.textContent = criticas || '';
      navBadge.style.display = criticas > 0 ? 'inline-flex' : 'none';
    }
  },
...
const Charts = {
  ...
  renderDonaEspecialistas(oficiales) {
    ...
    const bolsa      = oficiales.filter(o => Utils.esEnBolsa(o)).length;
  ...
  renderBarrasRegion(cuarteles, oficiales) {
    ...
    const optimos   = regiones.map(r => cuarteles.filter(c => c.region === r).reduce((s, c) => s + Utils.dotacionOptima(c), 0));
